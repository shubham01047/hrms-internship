...
class AdminDashboardApiController extends Controller
{
    public function index()
    {
        ...
        $topPerformer = Attendance::select('user_id', DB::raw('SEC_TO_TIME(SUM(TIME_TO_SEC(total_working_hours) + TIME_TO_SEC(overtime_working_hours))) as total_hours'))
            ...
            ->with('user')
            ->first();
        $attendancePercentage = $employeeCount > 0
            ? round(($todayPunchInCount / $employeeCount) * 100, 2)
            : 0;
        $projects = Project::with(['tasks.assignedUsers'])->get();
        $projectsThisWeek = Project::with(['tasks.assignedUsers'])
            ->whereBetween('deadline', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])
            ->get();
        $holidaysThisWeek = Holiday::whereBetween('date', [Carbon::now(), Carbon::now()->endOfWeek()])
            ->orderBy('date', 'asc')
            ->get();
        ...
        $latePunches = Attendance::with('user')
            ...
            ->get()
            ->filter(fn($attendance) => Carbon::parse($attendance->punch_in)->gt($officeStart));
        $lateCount = $latePunches->count();
        return response()->json([
            'employees' => $employees,
            'birthdays_tomorrow' => $employeesWithBirthdayTomorrow,
