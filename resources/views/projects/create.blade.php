<x-app-layout>
    <x-slot name="header">
        <h1 class="text-4xl font-extrabold text-[var(--secondary-text)] bg-[var(--secondary-bg)] px-6 py-4 rounded-lg shadow-lg">Create New Project</h1>
        ...
        <div class="w-full max-w-3xl animate-fade-in animate-delay-200">
            <div class="form-container">
                <div class="form-header">
                    ...
                    <p class="text-gray-600">Fill in the details to create a new project</p>
                </div>
                <div class="form-body">
                    <form method="POST" action="{{ route('projects.store') }}" class="space-y-0">
                        @csrf
                                ...
                                @foreach ($users as $user)
                                    <label for="member-{{ $user->id }}" class="checkbox-container flex items-center">
                                        <input
                                            ...
                                            value="{{ $user->id }}"
                                            id="member-{{ $user->id }}"
                                            class="hidden peer" {{-- Hidden, but still a peer --}}
