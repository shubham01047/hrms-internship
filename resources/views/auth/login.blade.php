<x-guest-layout>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-6">
                    ...
                    <div class="flex items-center justify-between text-sm">
                        <label class="flex items-center space-x-2">
                            <input type="checkbox" name="remember" class="w-4 h-4 text-red-600 rounded"> {{-- Updated checkbox color --}}
                            <span>Remember me</span>
                        </label>
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-red-600 hover:underline">Forgot Password?</a> {{-- Updated link color --}}
                        @endif
                    </div>
    ...
    <style>
        .gradient-bg {
            background: linear-gradient(135deg, #ff2626, #ff6969, #ff2626); /* Updated to red gradient */
