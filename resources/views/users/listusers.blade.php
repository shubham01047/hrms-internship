            ...
            <div class="max-w-7xl mx-auto">
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-0">
                    <div class="flex items-center space-x-3 sm:space-x-4 text-center sm:text-left">
                        ...
                        <div>
                            <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold leading-tight" style="color: var(--primary-text);">
                                {{ __('User Management') }}
                            </h1>
                    ...
                    <div class="flex items-center space-x-2 text-gray-600 bg-gray-100 px-3 py-2 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl text-xs sm:text-sm w-full sm:w-auto justify-center">
                        ...
                        <span class="font-bold text-blue-600">{{ $users->total() ?? $users->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
                                    ...
                                    <tr class="user-row hover:bg-gray-50 transition-all duration-200 ease-in-out" 
                                        ...
                                        <td class="px-4 py-3 sm:px-8 sm:py-6 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 mr-1.5 sm:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                                                </svg>
                                                ...
                                                @if($user->roles->isNotEmpty())
                                                    <div class="flex flex-wrap gap-1.5 sm:gap-2">
                                                        @foreach($user->roles->take(2) as $role)
                                                            <span class="inline-flex items-center px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800 role-badge">
                                                                {{ $role->name }}
                                                            </span>
                                                        @endforeach
                                                        @if($user->roles->count() > 2)
                                                            <span class="inline-flex items-center px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-600">
                                                                +{{ $user->roles->count() - 2 }} more
                                                            </span>
                                                        @endif
                                                    </div>
                                                @else
                                                    <span class="inline-flex items-center px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-500">
                                                        No roles assigned
                                                    </span>
                                                @endif
                                            </div>
                            ...
                            <tr id="noResultsRow" class="hidden">
                                <td colspan="6" class="px-4 py-8 sm:px-8 sm:py-16 text-center">
                                    <div class="flex flex-col items-center justify-center space-y-3 sm:space-y-4">
                                        <div class="w-14 h-14 sm:w-16 sm:h-16 bg-gray-100 rounded-full flex items-center justify-center">
                                            <svg class="w-7 h-7 sm:w-8 sm:h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                                            </svg>
                                        </div>
                                        <div class="text-base sm:text-lg font-medium text-gray-900">No users found</div>
                                ...
                                <tr id="emptyStateRow">
                                    <td colspan="6" class="px-4 py-8 sm:px-8 sm:py-16 text-center">
                                        <div class="flex flex-col items-center justify-center space-y-3 sm:space-y-4">
                                            ...
                                            <div class="text-base sm:text-lg font-medium text-gray-900">No users found</div>
                                            <div class="text-xs sm:text-sm text-gray-500">Users will appear here once they are registered.</div>
                                        </div>
                                    </td>
                                </tr>
                            @endif
