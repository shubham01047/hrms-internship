        ...
        <div class="bg-white rounded-xl shadow-xl overflow-hidden border border-gray-200">
            ...
            <div class="p-6">
                @forelse ($attendances as $index => $attendance)
                    <div class="mb-6 bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200">
                        <!-- Employee Header -->
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500">Employee ID: EMP{{ str_pad($index + 1, 3, '0', STR_PAD_LEFT) }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="bg-red-50 p-4 rounded-lg border border-red-200">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-lg font-bold text-red-900">{{ $attendance['punch_out'] ?: 'Not punched out' }}</p>
                                            <p>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        @foreach ($attendance['breaks'] as $break)
                                            @php
                                                ...
                                                $breakConfig = [
                                                    'morning tea' => ['bg' => 'bg-amber-50', 'border' => 'border-amber-200', 'text' => 'text-amber-800', 'icon' => 'text-amber-600', 'badge' => 'bg-amber-100', 'emoji' => '☕'],
                                                    'lunch' => ['bg' => 'bg-green-50', 'border' => 'border-green-200', 'text' => 'text-green-800', 'icon' => 'text-green-600', 'badge' => 'bg-green-100', 'emoji' => '🍽️'],
                                                    'evening tea' => ['bg' => 'bg-purple-50', 'border' => 'border-purple-200', 'text' => 'text-purple-800', 'icon' => 'text-purple-600', 'badge' => 'bg-purple-100', 'emoji' => '🫖'],
                                                    'coffee break' => ['bg' => 'bg-orange-50', 'border' => 'border-orange-200', 'text' => 'text-orange-800', 'icon' => 'text-orange-600', 'badge' => 'bg-orange-100', 'emoji' => '☕'],
                                                    'prayer' => ['bg' => 'bg-blue-50', 'border' => 'border-blue-200', 'text' => 'text-blue-800', 'icon' => 'text-blue-600', 'badge' => 'bg-blue-100', 'emoji' => '🙏'],
                                                    'smoking' => ['bg' => 'bg-gray-50', 'border' => 'border-gray-200', 'text' => 'text-gray-800', 'icon' => 'text-gray-600', 'badge' => 'bg-gray-100', 'emoji' => '🚬'],
                                                    'personal' => ['bg' => 'bg-pink-50', 'border' => 'border-pink-200', 'text' => 'text-pink-800', 'icon' => 'text-pink-600', 'badge' => 'bg-pink-100', 'emoji' => '👤'],
                                                ];
                                                $config = $breakConfig[$breakType] ?? $breakConfig['personal'];
                                            @endphp
                                            
                                            <div class="p-3 rounded-lg border {{ $config['bg'] }} {{ $config['border'] }} hover:shadow-sm transition-all duration-200">
                                                <div class="flex items-center justify-between">
                                                    <div class="flex items-center space-x-2">
                                                        <span class="text-lg">{{ $config['emoji'] }}</span>
                                                        <div>
                                                            <p class="text-sm font-semibold {{ $config['text'] }} capitalize">{{ $break['type'] ?? 'Break' }}</p>
                                                            <p class="text-xs {{ $config['text'] }} opacity-75">Duration</p>
                                                        </div>
                                                    </div>
                                                    <div class="text-right">
                                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-bold {{ $config['badge'] }} {{ $config['text'] }}">
